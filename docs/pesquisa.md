...
## 8. Ronda de polimento pós-teste real: hostname, boot duplicado, machine-id, Firefox, idioma, App Center

Depois de testar as imagens finais (§7) no hardware de verdade, o usuário levantou uma lista de ajustes. Investigado e resolvido item a item:

**8.1 Hostname do Desktop errado (`q6a-server`).** Causa: o Desktop é construído *em cima* do rootfs do Server já configurado (que grava `q6a-server` fixo via `scripts/02b-configure-system.sh`) — nunca reexecutávamos essa etapa com nome diferente. Corrigido com [scripts/07a-fix-desktop-hostname.sh](../scripts/07a-fix-desktop-hostname.sh) (só Desktop).

**8.2 Entrada duplicada no menu do systemd-boot — causa raiz encontrada.** Inspecionando a ESP ao vivo (`/boot/efi/loader/entries/`, montada em `/boot/efi` conforme `/etc/fstab` — não em `/boot`), apareceram duas entradas pro mesmo kernel:
```
ubuntu-6.18.2-current-qcs6490.conf                          → "Ubuntu 26.04 LTS (Resolute Raccoon)"  (a nossa, escrita em provision-partitions.sh)
<machine-id>-6.18.2-current-qcs6490.conf                    → "Ubuntu 26.04.1 LTS"                    (gerada automaticamente)
```
A segunda traz o comentário `# File created by /usr/lib/kernel/install.d/90-loaderentry.install (systemd 259.5)` — é o **`kernel-install` do próprio systemd** (via `systemd-boot-tools`, que já vem instalado) criando sua entrada padrão (nome baseado no `machine-id`) sempre que uma trigger do dpkg dispara a rotina (acontece normalmente num `apt upgrade`, quando `/usr/lib/modules/<kver>` é tocado). Como o nosso arquivo não segue esse padrão de nome, o `kernel-install` não o reconhece e cria o dele em paralelo. `grub-efi-arm64` também está instalado no sistema, mas confirmado inofensivo aqui (seu hook só age se `/boot/grub/grub.cfg` existir, o que não é o nosso caso).

Solução implementada ([scripts/lib/inner-install-bootentry-dedupe.sh](../scripts/lib/inner-install-bootentry-dedupe.sh) + [scripts/06b-install-bootentry-dedupe.sh](../scripts/06b-install-bootentry-dedupe.sh)): mantém nossa entrada (necessária pro primeiro boot funcionar) e instala um par de unidades systemd — `q6a-dedupe-boot-entry.path` (observa mudanças em `/boot/efi/loader/entries`) + `.service` (oneshot, também habilitado em `multi-user.target` pra rodar 1x por boot) — que detecta quando o `kernel-install` cria a entrada oficial pro nosso kernel (nome `<32 hex>-<kver>.conf`) e remove a nossa, sobrando só uma. Testado offline (simulação local do diretório de entradas): detecta, remove, é idempotente, ignora entradas de outros kernels.

**8.3 Bug relacionado, mais sério, encontrado de bônus: `machine-id` fixo e compartilhado.** Ao investigar o item acima, percebemos que o ID da entrada duplicada (`9b9d4295...`) era **exatamente o mesmo `/etc/machine-id`** presente no nosso rootfs — ou seja, o pacote `systemd` gera um machine-id de verdade durante o `apt-get install` dentro do chroot (via `systemd-machine-id-setup` no postinst), e como nunca zeramos isso depois, **todo aparelho gravado com a mesma imagem saía com a mesma identidade de máquina** (afeta D-Bus, journal, e qualquer coisa que use isso como "impressão digital" do aparelho). Corrigido com [scripts/08-clear-machine-id.sh](../scripts/08-clear-machine-id.sh) — zera o arquivo (0 bytes, convenção padrão do systemd pra imagens-molde) como o ÚLTIMO passo do build, imediatamente antes de `03-assemble-image.sh`; cada aparelho gera o seu próprio no primeiro boot real.

**8.4 Sem navegador — Firefox nativo via repo oficial da Mozilla.** O usuário tinha instalado o Firefox manualmente pra testar, e isso confirmou uma suspeita: o "Firefox" padrão do Ubuntu moderno é só um pacote de transição que puxa o **snapd inteiro** + Firefox via snap (confirmado: `snap list` mostrou `firefox`, `snapd`, `core24`, `gnome-46-2404`, `gtk-common-themes`, `mesa-2404`, `bare` todos instalados de uma vez). Em vez disso, [scripts/lib/inner-install-firefox.sh](../scripts/lib/inner-install-firefox.sh) instala o `.deb` nativo arm64 direto do repositório oficial da Mozilla (`packages.mozilla.org`, chave verificada contra o fingerprint oficial `35BAA0B33E9EB396F59CA838C0BA5CE6DC6315A3`, com pin de prioridade 1000 pra nunca perder pro pacote de transição da Ubuntu) — sem snap, sem snapd.

**8.5 Idioma: sistema não tinha NENHUM locale gerado.** `/etc/default/locale` estava vazio (nem C.UTF-8 de verdade) — o "inglês" que aparecia era só o fallback cru do locale POSIX, não um `en_US.UTF-8` de verdade. [scripts/06a-fix-locale.sh](../scripts/06a-fix-locale.sh) gera e configura `en_US.UTF-8` como padrão (Server e Desktop; inglês escolhido como padrão do release público — usuários pt-BR podem trocar depois).

**8.6 "Centro de Aplicativos" mencionado na mensagem de boas-vindas, mas ausente.** O `gnome-initial-setup` (instalado) usa o texto padrão da Ubuntu, que menciona um App Center — mas o "App Center" atual da Ubuntu é distribuído como **snap**, o que reintroduziria o mesmo peso que acabamos de evitar no Firefox. [scripts/07c-install-gnome-software.sh](../scripts/07c-install-gnome-software.sh) instala o `gnome-software` clássico (`.deb` nativo, ~13MB, sem snapd) como alternativa — cobre a necessidade real (uma forma gráfica de instalar software) sem reintroduzir a dependência.

**8.7 GPU "genérica" no app Recursos — falso alarme.** Confirmado via `/sys/class/drm/card1/device/uevent`: `DRIVER=msm_dpu`, `qcom,sc7280-dpu` — driver real, vinculado e funcionando. O app GNOME "Recursos" só não tem essa GPU embarcada Qualcomm cadastrada no banco de nomes bonitos dele; não é falha de driver.

**8.8 Baseline atualizado pra Ubuntu 26.04.1 LTS.** [scripts/lib/inner-apt-upgrade.sh](../scripts/lib/inner-apt-upgrade.sh) + [scripts/06c-apt-upgrade.sh](../scripts/06c-apt-upgrade.sh) rodam `apt update && apt upgrade` como parte do build — mas só DEPOIS do item 8.2 (serviço de deduplicação) já estar instalado, senão herdaríamos a entrada duplicada de cara. Confirmado via `/etc/os-release`: `PRETTY_NAME="Ubuntu 26.04.1 LTS"` nos dois rootfs.
